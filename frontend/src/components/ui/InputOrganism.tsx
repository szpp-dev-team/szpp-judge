...
export const InputOrganism = <T extends ZodTypeAny>({
  ...
  const desc: string | undefined = schema.shape?.[name]?.description ?? schema._def.schema?.shape?.[name]?.description;

  return (
    <FormControl isInvalid={errors?.[name] != null} mb={12} position="relative" textAlign="left">
