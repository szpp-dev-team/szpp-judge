...
func (uq *UserQuery) QuerySubmits() *SubmitQuery {
...
func (uq *UserQuery) QueryContests() *ContestQuery {
	query := (&ContestClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) QueryContestUser() *ContestUserQuery {
	query := (&ContestUserClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithSubmits(opts ...func(*SubmitQuery)) *UserQuery {
	query := (&SubmitClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithContests(opts ...func(*ContestQuery)) *UserQuery {
	query := (&ContestClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithContestUser(opts ...func(*ContestUserQuery)) *UserQuery {
	query := (&ContestUserClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withContestUser = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withSubmits; query != nil {
	...
	if query := uq.withContests; query != nil {
	...
	if query := uq.withContestUser; query != nil {
		if err := uq.loadContestUser(ctx, query, nodes,
			func(n *User) { n.Edges.ContestUser = []*ContestUser{} },
			func(n *User, e *ContestUser) { n.Edges.ContestUser = append(n.Edges.ContestUser, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadSubmits(ctx context.Context, query *SubmitQuery, nodes []*User, init func(*User), assign func(*User, *Submit)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Submit(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SubmitsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_submits
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_submits" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (uq *UserQuery) loadContests(ctx context.Context, query *ContestQuery, nodes []*User, init func(*User), assign func(*User, *Contest)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*User)
	nids := make(map[int]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.ContestsTable)
		s.Join(joinT).On(s.C(contest.FieldID), joinT.C(user.ContestsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.ContestsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.ContestsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (uq *UserQuery) loadContestUser(ctx context.Context, query *ContestUserQuery, nodes []*User, init func(*User), assign func(*User, *ContestUser)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(contestuser.FieldUserID)
	}
	query.Where(predicate.ContestUser(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ContestUserColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
